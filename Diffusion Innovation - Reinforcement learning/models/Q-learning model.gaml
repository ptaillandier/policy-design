	...
	action simulate(int i, list<farmer> farmers) {
		...
		loop while:time_sim_i < end_simulation_after {
			...
			if (int(month_) > month) {
				month <- int(month_);
				if (month mod 6) = 0 {
					//the_institution.last_turn[i] <- (time_sim_i + 6 #month) > end_simulation_after;
...
species institution schedules: container(reinforcement_learning ? [] : shuffle(institution)){
	...
	//	write sample(num_adopters) + " " + sample(previous_adopters_nb) + " " + sample(mean_intentions_sim) + " "+ sample(previous_mean_intention);
		...
		float reward <- last_turn[id] ? num_adopters[id]/number_farmers : 0.0;// ((num_adopters[id] - previous_adopters_nb[id])/number_farmers) : 0.0;// + (mean_intentions_sim[id] - previous_mean_intention[id]);
		//if last_turn[id] {write sample(reward);
	...
	//	previous_mean_intention[id] <- mean_intentions_sim[id];
		...
		//write "avant: " + b + " apres: " + q[current_state][last_action] + " new state max:" + max(q[new_state].values);
	}
				...
				loop act over: acts_prim {
					switch act {
						match FINANCIAL_SUPPORT {
							do financial_support(id,0.3);
						}
						match TRAINING {
							do training(id,0.3, 20);
						}
						match ENV_SENSIBILISATION {
							do environmental_sensibilisation(id,0.5, 20);
						}
					}
				}
			}
...
experiment learn_policy_batch type: batch until: cycle > 0{
	action _init_ {
		create simulation with: (reinforcement_learning:true, nb_replications:5, save_result_in_file:true);
	}
}
